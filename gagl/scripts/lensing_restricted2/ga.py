...
def store_statistics():
	...
	for i in range(len(minc)):
		...
		outstr2 += str (meanc[i] )
		outstr2 += "\n"
		#outstr3 = str( get_lensdata(minchrom[i]) )
		outstr3 = ''
...
def store_population(pop, generation):
	...
	for i in range(popsize):
		outstr1 = ''
		outstr1 += repr(refparms[0])
		outstr1 += "\t"
		outstr1 += repr(refparms[1])
		outstr1 += "\t"
...
def create_population(n, parmranges):
	...
	for i in range(n):
		...
		fitness = 0
		#assign
		#pop[2*i] = create_random_chromosome(parmranges)
		#print i, " ", j	
		pop[2*i] = []
...
def relative_error(chrom):
	...
	"""
	data = lensing(refparms[0], refparms[1], chrom[0], chrom[1])
	res = extract(data)
	print ref
	print res
	"""
...
#-------------------------------------------------------------------------------
def plot_reference():
	return lensplot(.0160, 0.0047, 1.5, 0.5, screenplot = False, pdfplot = True)
...
def ga(psize, mrate, xrate, iters, ranges, tofile = False):
	...
	pop = create_population_random(psize, ranges)
	...
	for i in range(1,iters):		
		...
		# store and print
		#lensplot(pop[0][0], pop[0][1], pop[0][2], pop[0][3], screenplot = False, pdfplot = True)		
		#lensplot(0.016, 0.0047, pop[0][0], pop[0][1], False, True)		
...
# driver
#-------------------------------------------------------------------------------
#best_fitness = ga(psize=12, mrate=0.4, xrate=0.6, iters = 100, ranges=[[0.0,2.0],[0.0,0.4]] , tofile=True)
#best_fitness = ga(psize=2, mrate=0.4, xrate=0.6, iters = 1, ranges=[[0.0,0.01],[0.0,0.075]] , tofile=True)
#best_fitness = ga(psize=12, mrate=0.4, xrate=0.6, iters = 10, ranges=[[0.0,0.16],[0.0,0.047],[0,10.5],[0,5.0]] , tofile=True)
#best_fitness = ga(psize=12, mrate=0.4, xrate=0.6, iters = 100, ranges=[[0.0,0.16],[0.0,0.047], [1.5,1.5], [0.5,0.5]] , tofile=True)
#best_fitness = ga(psize=12, mrate=0.4, xrate=0.6, iters = 100, ranges=[[0.016,0.016],[0.0047,0.0047], [0,10.5], [0.0,5.0]] , tofile=True)
#best_fitness = ga(psize=12, mrate=0.6, xrate=0.5, iters = 2, ranges=[[0.016,0.016],[0.0047,0.0047], [0,10.5], [0.0,5.0]] , tofile=True)
#best_fitness = ga(psize=12, mrate=0.2, xrate=0.5, iters = 30, ranges=[[0.0,0.16],[0.0,0.047], [1.5,1.5], [0.5,0.5]] , tofile=True)
#best_fitness = ga(psize=12, mrate=0.4, xrate=0.5, iters = 50, ranges=[[0.0,0.16],[0.0,0.047]] , tofile=True)
#best_fitness = ga(psize=12, mrate=0.4, xrate=0.5, iters = 50, ranges=[[0.0,10.5],[0.0,5.0]] , tofile=True)

#best_fitness = ga(psize=48, mrate=0.3, xrate=0.5, iters = 1, ranges=[[1.0,10.5],[0.0,5.0]] , tofile=True)
niters = 1
seed(454)
fits = []
for i in range(niters):
	best_chrom, best_fitness = ga(psize=2, mrate=0.3, xrate=0.5, iters = 5, ranges=[[1.0,10.5],[0.0,5.0]] , tofile=True)
