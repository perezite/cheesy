...
function set_question () {
	...
	if ( nextOne == questions.length ) alert ( 'Es liegen keine weiteren Fragen vor.')
	else {
...
function feedback ( isitright ) {
	...
	if ( isitright ) {
	  ...
	  if ( questions[nextOne][studentanswer + 1].length > 1 )
		...
		if ( secondRun ) {
		  if ( Math.abs(questions[nextOne][7]) != 3 ) showComment += ' - die richtige Antwort ist ' + questions[nextOne][6] ;
